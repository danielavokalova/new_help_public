...
export default function PortalLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!isPortalRoot) {
      ...
      if (match) setExpandedCat(match.name);
    }
  }, [pathname, isPortalRoot]);

  function handleCatClick(cat: Category) {
    if (!isPortalRoot) router.push(cat.href);
    setSelectedCat(cat);
  ...
  return (
    <PortalContext.Provider value={{ selectedCat, setSelectedCat, openContact: () => setShowContact(true) }}>
      ...
      <div className={s.mobileTopBar}>
        ...
        <span className={s.mobileTopBarTitle}>Meetings Help</span>
      </div>

      <div className={s.portalRoot}>

        {showMobileSidebar && (
          <div className={s.sidebarBackdrop} onClick={() => setShowMobileSidebar(false)} />
        )}
                ...
                <form onSubmit={handleFormSubmit}>
                  ...
                  <div className={s.formGroup}>
                    ...
                    <input id="ct-subject" name="subject" required className={s.formInput}
                      value={form.subject} onChange={handleFormChange} placeholder="e.g. Approver not receiving notifications" />
                  </div>
                  <div className={s.formGroup}>
