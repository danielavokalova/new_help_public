# How to handle bookings with clients

<!-- tags: booking, client, notification, PNR, fraud, verification, payment -->

GOL IBE uses a two-step notification approach to protect against fake bookings and fraud. Here is what happens after a client completes a booking and what your role is.

## What the client sees

After finishing the booking process the client sees a neutral "thank you" confirmation on screen. At the same time a simple notification is sent to the client's email — **without a PNR or any attachment**. This is intentional: it prevents abusers from harvesting PNR numbers.

## What you receive

The full notification — with PNR and the ticket itinerary attached — is delivered **only to you (the agency)**.

## What to do next

1. **Verify the client.** When you receive the booking notification, contact the client and confirm they are a real person, not a bot or abuser.
2. **Discuss payment.** Use this contact to agree on the payment method and any other details.
3. **Send the full confirmation.** Once you are satisfied the client is genuine, forward the complete notification (with the ticket itinerary) from your mailbox — or skip this step if you prefer.
4. **Issue the ticket.** As soon as the client pays, provide the flight ticket as usual.

> Skipping the forwarding step is fine — it is entirely up to you how you handle client communication after verification.

---

📩 Feel free to contact us at: [gol_support@cee-systems.com](mailto:gol_support@cee-systems.com)
